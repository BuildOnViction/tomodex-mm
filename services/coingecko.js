...
const getLatestPrice = async (p = false) => {
    try {
        ...
        const response = await axios.get(`https://api.coingecko.com/api/v3/simple/price?ids=dxdao&vs_currencies=usd`)
        gPrice[p] = response.data['dxdao'].usd
...
const getUSDPrice = async (p = false) => {
    ...
    try {
        ...
        if (baseSymbol != 'USDT' && baseSymbol != 'USD') {
            let response = await axios.get(`https://api.coingecko.com/api/v3/simple/price?ids=dxdao&vs_currencies=usd`)
            let tokenPrice = response.data['dxdao'].usd
